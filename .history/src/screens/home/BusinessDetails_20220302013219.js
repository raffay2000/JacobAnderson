...
const BusinessDetails = ({route}) => {
    ...
    const [isFavorite, setIsFavorite]=useState(businessDetails.favorite);
            ...
            "MessagesStack", {
                ...
                params:{
                    person:{
                        ...
                        image: imagePath+'/'+businessDetails.images[0].user_image,
    ...
    useEffect(()=>{console.log('event changes')},[BusinessEvent])
    const onLikePress = (event, index) => {
        ...
        // console.log('check fav', events[index])
    ...
    const SpecificBusinessEvents = () => {
        ...
        .then(function (response) {
            ...
            if(response.data.success){
                eventImagePath=response.data.image_path;
                setSuccess(true),
                setError(false)
            }
            else{
                ...
                <FlatList
                    ...
                    renderItem={({item,index}) =>
                        <ImageCard
                            ...
                            image={{uri: item.images? eventImagePath+"/"+item.images[0].event_image:"abc"}}
                            title={item.name}
    ...
    const onBusinessLikePress = () => {
        var data = new FormData();
        data.append('user_id', user.id);
        data.append('business_id', EventID); 
    ...
    const Get_Review = ()=> {
        ...
        var config = {
            method: 'get',
            url: API+'get-reviews/'+businessDetails.id,
            headers: {
                Authorization: "Bearer "+token
            }
          };
          ...
          .then(function (response) {
              ...
              if(response.data.success){
                  setLoader(false)
                  setFailed(false)
                  setGetReview(response.data.data.reviews)
              }
              else{
                  setLoader(false)
                  setFailed(true)
    ...
    return(
        <Container>
            <SimpleHeader
                ...
                icon2="heart"
                icon2Press={onBusinessLikePress}
