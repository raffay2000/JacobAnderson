...
class Login_Page extends Component{
    ...
    onSignInPress = ()=> {
        ...
        if(this.state.Email == "" || this.state.Password == ""){
            Toast.show({text1: "Enter Email and Password"})
        }
        else{
            this.props.login(this.state.Email, this.state.Password,"","", this.context)
    ...
    SignInWithFaceBook = async() => {
        ...
        //         this.props.socialLogin(id, name, this.context)
        //     } 
        //     else {
    ...
    render() {
        return (
            <View style={[styles.container,{backgroundColor:white}]}>
                <ScrollView style={{width: '100%'}} contentContainerStyle={{alignItems: 'center', backgroundColor: 'red'}}>
                <Image 
                    source={require("../../assets/images/logo.png")}
                    style={{width:"45%", height:hp("16%")}}
                />
...
function mapDispatchToProps(dispatch) {
    return{
        login: (email, password, fb_name, fb_id, context)=> dispatch(Login(email, password,fb_name, fb_id, context)),
        socialLogin: (id, name, context)=> dispatch(socialLogin(id, name, context)),
