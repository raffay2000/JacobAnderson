...
class Edit_Business extends Component{
    ...
    pickImage = (index) => async () => {
        ...
        if(!result.cancelled){
            if(this.state.AddImg[index]?.image.includes(this.state.path)){
                console.log('deleted image added', this.state.AddImg[index].id)
                var deleteImage = this.state.deleteImage;
                deleteImage.push(this.state.AddImg[index].id)
    ...
    OnEdit_Business = () => {
        // console.log('delete images', this.state.deleteImage)
        // console.log('images array', this.state.AddImg)
        Keyboard.dismiss();
        ...
        data.append('deleteImage', JSON.stringify(that.state.deleteImage));
        that.state.AddImg.forEach((element, i )=> {
            if(!element.image.includes(that.state.path)){
                const newFile = {
                    name: "images.jpg",
                    type: "images/jpeg",
                    uri: Platform.OS === "android" ?element.image : element.image.replace("file://", "")
                }  
        ...
        //             Success:true
        //         })
        //         await AsyncStorage.setItem('user', JSON.stringify(response.data.user), (err)=> err?true:false)
...
Edit_Business.contextType = AuthContext;
