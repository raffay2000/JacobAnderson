...
class Edit_Business extends Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            Success:false,
            path: null,
            deleteImage: [] // ek or array banega js m new images aengi // or condition b lagegi image reselect karne pr
        }
    }
    ...
    componentDidMount(){
        ...
        AsyncStorage.getItem("user", async (err, data) =>{
            ...
            this.setState({
                ...
                AddImg:business.images.map(item=> ({id: item.id, image:image_path+'/'+item.user_image})),
                path: image_path
            })
        })
    }
    
    pickImage = ( item, index) => async () => {
    ...
    renderImages() {
        var images = [];
        for(var i=0; i<4; i++){
            console.log('index', this.state.AddImg[i])
            images.push(
                <ImgComp image={this.state.AddImg[i] ? this.state.AddImg[i].image : null} Onpress={this.pickImage(this.state.AddImg[i], i)} />
            )
        }
        return images;
    }
             ...
             <Container>
               ...
               <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{alignItems:"flex-start"}} >
                    ...
                    <View style={[styles.row, {marginTop:hp("2%"),width:"100%", justifyContent:"space-between"}]}  >
                        ...
                        {/* <ImgComp image={this.state.AddImg[0].image ? this.state.AddImg[0].image : null} Onpress={this.pickImage(0)} />
                        <ImgComp  image={this.state.AddImg[1].image ? this.state.AddImg[1].image : null} Onpress={this.pickImage(1)} />
                        <ImgComp  image={this.state.AddImg[2].image ? this.state.AddImg[2].image : null} Onpress={this.pickImage(2)} />
                        <ImgComp  image={this.state.AddImg[3].image ? this.state.AddImg[3].image : null} Onpress={this.pickImage(3)} /> */}
