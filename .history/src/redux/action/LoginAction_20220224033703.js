    ...
    return (dispatch)=>{
        ...
        .then(async function (response) {
            if(response.data.success){
                ...
                await AsyncStorage.setItem('imagePath', response.data.image_path, (err)=> err?true:false)
                await AsyncStorage.setItem('paid', 'false', (err)=> err?true:false)
                await AsyncStorage.setItem('token', response.data.token, (err)=> err?true:false)
                context.updateState()
            }
            else{
                Toast.show({text1: "Incorrect Email and Password"})
                dispatch({type: LOGIN_FAILED, error:"Incorrect Email and Password" })
                console.log(JSON.stringify(response.data.data));
            }
...
export const putUserToken = (token) => {
    return{
        type:TOKEN,
        payload:token   
    }
